...
DEFAULT_XIAOMI_CHAT_URL = f"{DEFAULT_XIAOMI_BASE_URL}/chat/completions"
DEFAULT_XIAOMI_MODEL = "mimo-v2.5"
DEFAULT_PROVIDER_LABEL = "OpenAI-compatible"
XIAOMI_PROVIDER_LABEL = "Xiaomi MiMo"
...
class LLMConfigError(RuntimeError):
    """Raised when the local runtime is missing LLM provider credentials."""


class LLMResponseError(RuntimeError):
    """Raised when LLM provider cannot return a valid StrategySpec payload."""
...
class LLMConfig:
    ...
    def from_env(cls, environ: Mapping[str, str] | None = None) -> "LLMConfig":
        ...
        api_key = (env.get("LLM_API_KEY") or env.get("XIAOMI_API_KEY") or env.get("MIMO_API_KEY") or "").strip()
        if not api_key:
            raise LLMConfigError("Missing LLM_API_KEY in the local environment.")

        provider_label = (
            env.get("LLM_PROVIDER_LABEL")
            or (XIAOMI_PROVIDER_LABEL if env.get("XIAOMI_API_KEY") or env.get("MIMO_API_KEY") else DEFAULT_PROVIDER_LABEL)
        ).strip()

        chat_url = (
            env.get("LLM_CHAT_COMPLETIONS_URL")
            or env.get("XIAOMI_CHAT_COMPLETIONS_URL")
            or env.get("MIMO_CHAT_COMPLETIONS_URL")
            or ""
        ).strip()
        if not chat_url:
            base_url = (env.get("LLM_BASE_URL") or env.get("XIAOMI_BASE_URL") or env.get("MIMO_BASE_URL") or "").strip()
            if not base_url and (env.get("XIAOMI_API_KEY") or env.get("MIMO_API_KEY")):
                base_url = DEFAULT_XIAOMI_BASE_URL
            if not base_url:
                raise LLMConfigError("Missing LLM_BASE_URL or LLM_CHAT_COMPLETIONS_URL in the local environment.")
            chat_url = build_chat_url(base_url)

        model = (env.get("LLM_MODEL") or env.get("XIAOMI_MODEL") or env.get("MIMO_MODEL") or "").strip()
        if not model and (env.get("XIAOMI_API_KEY") or env.get("MIMO_API_KEY")):
            model = DEFAULT_XIAOMI_MODEL
        if not model:
            raise LLMConfigError("Missing LLM_MODEL in the local environment.")
        return cls(api_key=api_key, chat_url=chat_url, model=model, provider_label=provider_label)


def get_llm_status(environ: Mapping[str, str] | None = None) -> dict[str, Any]:
    env = os.environ if environ is None else environ
    try:
        ...
        chat_url = (
            ...
            or env.get("XIAOMI_CHAT_COMPLETIONS_URL")
            or env.get("MIMO_CHAT_COMPLETIONS_URL")
            or (build_chat_url(base_url) if base_url else "")
        ).strip()
        return {
            "configured": False,
            "api_key": "missing",
            "provider_label": (env.get("LLM_PROVIDER_LABEL") or DEFAULT_PROVIDER_LABEL).strip(),
            "model": (env.get("LLM_MODEL") or env.get("XIAOMI_MODEL") or env.get("MIMO_MODEL") or "").strip(),
            "chat_url": chat_url,
            "error": str(exc),
        }
...
def parse_strategy_request_with_llm(
    request: str,
    ...
    payload = build_llm_payload(request=request, baseline_spec=baseline_spec, model=llm_config.model)
    headers = {
    ...
    response = (transport or post_json)(llm_config.chat_url, headers, payload, llm_config.timeout_seconds)
...
def build_llm_payload(request: str, baseline_spec: dict[str, Any], model: str) -> dict[str, Any]:
    system_prompt = (
...
def post_json(url: str, headers: dict[str, str], payload: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    try:
        ...
        if "LLM provider request failed:" not in str(exc):
            raise
        try:
...
def post_json_requests(url: str, headers: dict[str, str], payload: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    ...
    try:
        ...
        raise LLMResponseError(f"LLM provider request failed: {exc}") from exc

    if response.status_code >= 400:
        raise LLMResponseError(f"LLM provider HTTP {response.status_code}: {redact_secret(response.text, api_key)}")

    try:
        return response.json()
    except ValueError as exc:
        raise LLMResponseError("LLM provider returned non-JSON HTTP response.") from exc


def post_json_curl(url: str, headers: dict[str, str], payload: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    curl = shutil.which("curl.exe") or shutil.which("curl")
    if not curl:
        raise LLMResponseError("LLM provider request failed and curl fallback is unavailable.")
    ...
    if completed.returncode != 0:
        ...
        raise LLMResponseError(f"LLM provider curl fallback failed: {redact_secret(message, api_key)}")
    ...
    if isinstance(data, dict) and "error" in data:
        raise LLMResponseError(f"LLM provider error: {redact_secret(response_text, api_key)}")
    return data
...
def post_json_urllib(url: str, headers: dict[str, str], payload: dict[str, Any], timeout_seconds: float) -> dict[str, Any]:
    ...
    try:
        ...
        raise LLMResponseError(f"LLM provider HTTP {exc.code}: {redact_secret(body, api_key)}") from exc
...
def extract_message_content(response: dict[str, Any]) -> str:
    try:
        ...
        raise LLMResponseError("LLM provider response did not contain choices[0].message.content.") from exc
    if not isinstance(content, str) or not content.strip():
        raise LLMResponseError("LLM provider returned an empty message content.")
    return content
...
def parse_json_object(content: str) -> dict[str, Any]:
    ...
    try:
        ...
        if start < 0 or end <= start:
            raise LLMResponseError("LLM provider response was not a JSON object.")
        value = json.loads(text[start : end + 1])

    if not isinstance(value, dict):
        raise LLMResponseError("LLM provider response JSON must be an object.")
    return value
...
def build_chat_url(base_url: str) -> str:
    ...
    if not url:
        return DEFAULT_XIAOMI_CHAT_URL
