    ...
    return [
        {
            ...
            "user_id": user_id,
            "request": request,
            "result": run_skill_generation(request=request, user_id=user_id),
        }
...
def print_summary(results: list[dict[str, Any]]) -> None:
    for item in results:
        result = item["result"]
        draft = result.get("generated_skill_draft") or {}
        review = result.get("draft_review") or {}
        print("=" * 80)
        print(f"{item['case_id']}: user_id={item['user_id']}")
        print(f"request: {item['request']}")
        print(f"status: {result['status']}")
        print(f"draft_name: {draft.get('name') or 'none'}")
        print(f"review_status: {review.get('status')}")
        print(f"auto_enabled: {result['final_output'].get('auto_enabled')}")
        print(f"writes_runtime_config: {result['final_output'].get('writes_runtime_config')}")
        print(f"next_lab: {result['next_lab']}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Lab 07 Skill Generation demo.")
