...
def print_summary(results: list[dict[str, Any]]) -> None:
    for item in results:
        ...
        print(f"routing_mode: {(result['strategy_spec'].get('routing_decision') or {}).get('mode')}")
        print("tool_trace:")
        if result["tool_trace"]:
            for event in result["tool_trace"]:
                output = event.get("output") or {}
                print(
                    f"- {event['tool_name']} | status={event['status']} | "
                    f"reason={event['selected_reason']} | result={output.get('summary', 'n/a')}"
                )
        else:
            print("- no tools called")
        print(f"candidate_evidence_count: {len(result['candidate_evidence'])}")
        print(f"risk_flags: {', '.join(result['final_output'].get('risk_flags', [])) or 'none'}")
        print(f"next_lab: {result['next_lab']}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Lab 03 Finance Tool Use Mock demo.")
