...
| 08 | MX Skills Adapter | 03 Tool Use / 06 MCP | mock-first adapter contract、`adapter_trace`、`safety_gate` 和 real provider stub | 已实现 |
...
- `safety_gate.real_provider_allowed=false`，真实 provider 需要未来明确人工确认、环境变量和手动集成测试。
...
- real adapter stub 调用时 blocked，并说明未来启用条件。
